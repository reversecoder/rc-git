...
public class GitBackend {
    ...
    public GitBackend(File workDir, String remote, String branch, ScmLogger log)
            throws GitException {
        this.log = log;
        this.remote = remote;
        this.branch = branch;
    ...
    public void pushAll(boolean skipEmptyCommit) throws GitException {
        ...
        if (skipEmptyCommit) {
            if (!run("commit", new String[] { "-m",
                    "[rc-git]" + " commit to branch " + branch + " " + timestamp }))
                throw new GitException("Unable to commit files");
        } else {
            if (!run("commit", new String[] { "--allow-empty", "-m",
                    "[rc-git]" + " commit to branch " + branch + " " + timestamp }))
                throw new GitException("Unable to commit files");
        }
